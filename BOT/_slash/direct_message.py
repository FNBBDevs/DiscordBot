...
class direct_message(Group):
    ...
    def __init__(self, tree : CommandTree, guild):
        ...
        async def user(interaction: discord.Interaction, member: discord.Member, message: str):
            # Create DMs if they don't already exist
            dms: discord.DMChannel = await member.create_dm()

            await dms.send(message)
            await interaction.response.send_message(content=f'Sent a DM to {member.mention}\n>>> {message}', ephemeral=True)
        ...
        async def all(interaction: discord.Interaction, message: str):
            ...
            await interaction.followup.send(content=f'Sent a DM to everyone\n>>> {message}', ephemeral=True)
