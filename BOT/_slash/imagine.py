...
class Imagine:
    ...
    def __init__(self, tree, guild, args=None):
        ...
        async def imagine(
            ...
            current_directory = f"{os.getcwd()}\\BOT\\_utils\\_tmp\\stable_diffusion"
            
            print(f"os.getcwd(): {current_directory}")
                        
            taken_ids = os.listdir(f"{current_directory}")
            
            print(f"os.listdir(): {taken_ids}")

            stable_id = uuid.uuid4().hex
            
            print(f"NEW ID: {stable_id}")
            
            while stable_id in taken_ids:
                stable_id = uuid.uuid4().hex
            
            print("ID AFTER CHECK:", stable_id)
            
            os.mkdir(f"{current_directory}\\{stable_id}")
            
            print(f"os.mkdir() + os.listdir(): {os.listdir(current_directory)}")
            ...
            if interaction.client._fnbb_globals.get("imagine_generating"):
                ...
                if images.value > 1:
                    ...
                    os.rmdir(f"{current_directory}\\{stable_id}")
                    await interaction.followup.send(
                        embed=generic_colored_embed(
                            title="✨ Stable Diffusion x FNBB ✨",
                ...
                with open(f"{current_directory}\\{stable_id}\\info.json", "w") as json_file:
                ...
                try:
                    ...
                    for idx, image in enumerate(response["images"]):
                        save_image(
                            path=f"{current_directory}\\{stable_id}\\{stable_id}_{idx}.png",
                            data=image,
                        )
