...
class info(Group):
    ...
    def __init__(self, tree: CommandTree, guild):
        ...
        async def server(interaction: discord.Interaction):
            ...
            embed.set_footer(text=f'Created {interaction.guild.created_at.strftime("%B %d, %Y")}', icon_url='https://bit.ly/3SJZQwM')
        ...
        async def user(interaction: discord.Interaction, user: discord.User = None, show_display_info: bool = False):
            ...
            embed = discord.Embed(title=name, description=desc, url=avatar.url, color=user.color)
            embed.set_image(url=avatar.url)
            embed.set_footer(text=f'Created {user.created_at.strftime("%B %d, %Y")}', icon_url='https://bit.ly/3SJZQwM')
