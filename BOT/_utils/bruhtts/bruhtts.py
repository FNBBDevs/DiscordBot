...
async def say(message: str, member: str, curr_dir: str):
    ...
    try:
        ...
        print(f"USER_SELECTED_MODEL: {USER_SELECTED_MODEL}")
        ...
        try:
            res1 = subprocess.run(" ".join(TTS_COMMAND), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            res2 = subprocess.run(INFER_COMMAND, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            return True
        except Exception as e:
            print(f"Error: {e}")
            return False
    except Exception as e:
        print(f"bruhtts ERROR: {e}")
