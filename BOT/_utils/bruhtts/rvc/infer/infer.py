...
class Config:
    ...
    def load_config_json() -> dict:
        ...
        for config_file in version_config_list:
            with open(f"{os.getcwd()}/BOT/_utils/bruhtts/rvc/configs/{config_file}", "r") as f:
...
def load_hubert(models_path):
    global hubert_model
    if models_path:
        models, _, _ = checkpoint_utils.load_model_ensemble_and_task(
            [models_path + "\\hubert_base.pt"],
            suffix="",
        )
    else:
        models, _, _ = checkpoint_utils.load_model_ensemble_and_task(
...
def vc_single(
    ...
    try:
        ...
        if not hubert_model:
            load_hubert(models_path=models_path)
        if_f0 = cpt.get("f0", 1)
...
def get_vc(weight_root, sid):
    ...
    if sid == "" or sid == []:
        ...
        if hubert_model is not None:
            ...
            cpt = None
    person = weight_root
    cpt = torch.load(person, map_location=lambda storage, loc: storage.cuda(0))
    tgt_sr = cpt["config"][-1]
    ...
    if config.is_half:
        ...
        net_g = net_g.float()
    vc = VC(tgt_sr, config, models_path=models_path)
    n_spk = cpt["config"][-3]
